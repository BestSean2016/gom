...
bool selectSymbol(const char *symbol, ENUM_TIMEFRAMES timeframe) {
  ...
  gSelectedData =
      mapRatesData.getSymbol(MARKET_FOREX_FURTURES, the_symbol, timeframe);
...
int forex_simulator_new_data(TickVector &ticks, const RatesData *rd, int len,
  ...
  double value[6];
  double newMean = rd->rs.statPriceDelta.mean / ((double)time_frames * 60);
  double newStdev =
      rd->rs.statPriceDelta.stdv / sqrt((double)time_frames * 60);
  for (int i = 0; i < len; ++i) {
    MqlTick tick;
    MathGaussianNoise3(value, newMean, newStdev);
    MathGaussianNoise3(value + 3, rd->rs.statVolume.mean,
                       rd->rs.statVolume.stdv);
    ...
    tick.ask_volume[0] =
        (value[3] < 0) ? 1 : value[3]; // + ticks[i - 1].ask_volume[0];
    tick.bid_volume[0] =
        (value[4] < 0) ? 1 : value[4]; // + ticks[i - 1].bid_volume[0];
    tick.last_volume =
        (value[5] < 0) ? 1 : value[5]; // + ticks[i - 1].last_volume;
...
void releaseTickVector(TickVector &ticks) { ticks.clear(); }
