 ...
 *The ticket number is a unique
 *  order identifier.
 * To find out from what list the order has been selected, its close time must
 *be analyzed. If the order close
 * time equals to 0, the order is open or pending and taken from the terminal
 *open orders list.
 *
 * One can distinguish an opened order from a pending order by the order type.
 *If the order close time does not
 * equal to 0, the order is a closed order or a deleted pending order and was
 *selected from the terminal history.
 *
 * They also differ from each other by their order types.
 * The OrderSelect() function copies order data into program environment and all
 *further calls of OrderClosePrice(),
 * OrderCloseTime(), OrderComment(), OrderCommission(), OrderExpiration(),
 *OrderLots(), OrderMagicNumber(),
 * OrderOpenPrice(), OrderOpenTime(), OrderPrint(), OrderProfit(),
 *OrderStopLoss(), OrderSwap(), OrderSymbol(),
 * OrderTakeProfit(), OrderTicket(), OrderType() functions return the data,
 *copied earlier. It means that in some
 * cases the order details (open price, SL/TP levels or expiration date) may
 *change and the data become non-actual.
 * It is strongly recommended to call the OrderSelect() function before request
 *the order data.
 *
 * Consecutive selection of orders using the SELECT_BY_POS parameter returns
 *information in the sequence in which
 *  it was received from the trading server. Sorting of the resulting list of
 *orders cannot be guaranteed.
 *
 ...
 * @param pool [in]  Optional order pool index. Used when the selected parameter
 *is SELECT_BY_POS. It can be
 *                   any of the following values:
 *                   MODE_TRADES (default)- order selected from trading
 *pool(opened and pending orders),
 *                   MODE_HISTORY - order selected from history pool (closed and
 *canceled order).
...
extern bool OrderSelect(int index,              // index or order ticket
                        int select,             // flag
 ...
 * generated.
 * Pending order expiration time can be disabled in some trade servers. In this
 ...
 * @param arrow_color [in]  Arrow color for StopLoss/TakeProfit modifications in
 * the chart. If the
 *  parameter is missing or has CLR_NONE value, the arrows will not be shown in
 * the chart.
 * @return If the function succeeds, it returns true, otherwise false. To get
 * the detailed error
 * information, call the GetLastError() function.
 */
extern bool OrderModify(int ticket,           // ticket
                        double price,         // price
 ...
 * parameter is missing or has CLR_NONE value closing arrow will not be drawn on
 * the chart.
 * @return Returns true if successful, otherwise false. To get additional error
 * information, one has to call the GetLastError() function.
 */
extern bool OrderClose(int ticket,        // ticket
                       double lots,       // volume
 ...
 * Bid (for selling) or Ask (for buying) can be used as open price. If operation
 * is performed with a security differing from the current one, the MarketInfo()
 * function must be used with MODE_BID or MODE_ASK parameter for the latest
 * quotes for this security to be obtained.
 * Calculated or unnormalized price cannot be applied. If there has not been the
 * requested open price in the price thread or it has not been normalized
 ...
 * the slippage parameter. If the requested price is out of date, but present in
 * the thread, the order will be opened at the current price and only if the
 * current price lies within the range of price+-slippage.
 * StopLoss and TakeProfit levels cannot be too close to the market. The minimal
 * distance of stop levels in points can be obtained using the MarketInfo()
 * function with MODE_STOPLEVEL parameter. In the case of erroneous or
 * unnormalized stop levels, the error 130 (ERR_INVALID_STOPS) will be
 * generated. A zero value of MODE_STOPLEVEL means either absence of any
 * restrictions on the minimal distance for Stop Loss/Take Profit or the fact
 * that a trade server utilizes some external mechanisms for dynamic level
 * control, which cannot be translated in the client terminal. In the second
 * case, GetLastError() can return error 130, because MODE_STOPLEVEL is actually
 * "floating" here.
 * At placing of a pending order, the open price cannot be too close to the
 * market. The minimal distance of the pending price from the current market one
 * in points can be obtained using the MarketInfo() function with the
 * MODE_STOPLEVEL parameter. In case of false open price of a pending order, the
 * error 130 (ERR_INVALID_STOPS) will be generated.
 * Applying of pending order expiration time can be disabled in some trade
 * servers. In this case, when a non-zero value is specified in the expiration
 ...
 * limited. If this limit has been exceeded, no new order will be opened (or no
 * pending order will be placed) and trade server will return error 148
 ...
 * parameter is missing or has CLR_NONE value opening arrow is not drawn on the
 * chart. default is clrNONE
 * @return Returns number of the ticket assigned to the order by the trade
 * server or -1 if it fails.
 * To get additional error information, one has to call the GetLastError()
 * function.
 */
extern int OrderSend(string symbol,               // symbol
                     int cmd,                     // operation
 ...
 * @return Returns an identifying (magic) number of the currently selected order
 */
ulong OrderMagicNumber();


/**
 * @brief OrdersHistoryTotal Returns the number of closed orders in the account history loaded into the terminal.
 * @return Returns the number of closed orders in the account history loaded into the terminal.
 */
extern int  OrdersHistoryTotal();
